...
class Resizer {
  ...
  createCover(filePath, options) {
    ...
    const dstPath =
      this.getPathWithoutExtension(filePath) + `${suffix}.${format}`;

    return lwip
      .openAsync(filePath)
      .then(image =>
        image.coverAsync(options.width, options.height, interpolation)
      )
  ...
  processImage(filePath, attachBase) {
    const jobs = _.map(this.options.tasks, task => {
      if (_.isObject(task.resize)) {
        return _ =>
          this.createResized(filePath, task.resize).then(
            path => (attachBase.resizedPath = path)
          );
      } else if (_.isObject(task.cover)) {
        return _ =>
          this.createCover(filePath, task.cover).then(
            path => (attachBase.coverPath = path)
          );
      }
    });
  ...
  single(filename) {
    return (req, res, next) => {
      if (!this.options.multer)
        return next(
          new Error('multer is undefined, please provide one at configuration.')
        );
      this.options.multer.single(filename)(req, res, err => {
        if (!_.isObject(req.file)) return next(err);

        this.processImage(req.file.path, req.file)
          .then(_ => next())
          .catch(next);
      });
    };
  ...
  array(filename, maxCount) {
    return (req, res, next) => {
      ...
      this.options.multer.array(filename, maxCount)(req, res, err => {
        if (!_.isObject(req.files)) return next(err);

        async
          .eachSeries(req.files, file => this.processImage(file.path, file))
          .then(_ => next())
          .catch(next);
      });
    };
